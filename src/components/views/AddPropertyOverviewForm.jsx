...
import { usePropertyAPI } from "@/services/usePropertyAPI"; // ✅ Import the hook
import { PROPERTY_TYPES } from "@/constants/propertyRequirements";
import { getRequirementTemplateByType } from "@/services/propertyRequirementsDummyStore";
import {
  buildDynamicRequirementFields,
  buildInitialRequirementAnswers,
  computeRequirementEvaluation,
  validateRequirementAnswers,
} from "@/services/propertyRequirementsUtils";
import DynamicRequirementFields from "@/components/views/DynamicRequirementFields";

const AddPropertyOverviewForm = forwardRef((props, ref) => {
  const fileInputRef = useRef(null);
  const [images, setImages] = useState([]);
  const [thumbnailIdx, setThumbnailIdx] = useState(0);
  const [isUploading, setIsUploading] = useState(false);
  const [requirementTemplate, setRequirementTemplate] = useState(null);
  ...
  const dynamicRequirementFields = useMemo(() => buildDynamicRequirementFields(requirementTemplate), [requirementTemplate]);

  useEffect(() => {
    if (!formData.propertyType) {
      setRequirementTemplate(null);
      setRequirementAnswers({});
      setRequirementErrors({});
      return;
    }

    const template = getRequirementTemplateByType(formData.propertyType);
    const fields = buildDynamicRequirementFields(template);
    setRequirementTemplate(template);
    setRequirementAnswers((prev) => buildInitialRequirementAnswers(fields, prev));
    setRequirementErrors({});
  ...
  const buildPayload = () => ({
    ...
    requirementAnswers,
    requirementEvaluation: computeRequirementEvaluation(dynamicRequirementFields, requirementAnswers),
    requirementTemplateVersion: requirementTemplate?.version || 1,
    imageUrls: images,
  ...
  const handleRequirementAnswerChange = (key, value) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const validationErrors = validateRequirementAnswers(dynamicRequirementFields, requirementAnswers);
    if (Object.keys(validationErrors).length > 0) {
      setRequirementErrors(validationErrors);
      return;
    }
  ...
  return (
    <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit}>
      ...
      <DynamicRequirementFields
        fields={dynamicRequirementFields}
        answers={requirementAnswers}
        errors={requirementErrors}
        onChange={handleRequirementAnswerChange}
      />
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex-1 flex flex-col gap-2">
          ...
          <select
            ...
            placeholder="Enter yield per annum"
