...
import useAccountDeletionAPI from "@/services/useAccountDeletionAPI";
  ...
  return (
    <div className="flex flex-col gap-5 animate-pulse">
      ...
      <div className="border-t border-opacityClr-20 pt-5">
        ...
        <div className="mt-3 h-10 w-28 rounded-md bg-gray-200" />
      </div>
    </div>
  );
}
...
const SupportSettingsPage = () => {
  const [activeTab, setActiveTab] = useState<"contact" | "support" | "deletions">("contact");
  ...
  const {
    ...
    isLoadingDetail: isLoadingDeletionDetail,
    refetchDetail,
      ...
      contactColumnHelper.display({
        ...
        cell: (info) => (
          <button
            ...
            className="inline-flex rounded-md p-2 text-primary-10 transition-colors hover:bg-neutral-100 cursor-pointer"
  ...
  const deletionColumnHelper = createColumnHelper<AccountDeletionRequest>();
  const deletionColumns = useMemo(
    () => [
      deletionColumnHelper.accessor("email", {
        header: "Email",
        cell: (info) => <span className="font-medium text-primary-10">{info.getValue() ?? "—"}</span>,
      }),
      deletionColumnHelper.accessor("reason", {
        header: "Reason",
        cell: (info) => <span className="text-sm text-opacityClr-80">{info.getValue() ?? "—"}</span>,
      }),
      deletionColumnHelper.accessor("status", {
        header: "Status",
        cell: (info) => <span className="text-sm text-opacityClr-80">{info.getValue() ?? "—"}</span>,
      }),
      deletionColumnHelper.accessor("createdAt", {
        header: "Date",
        cell: (info) => <span className="font-normal text-gray-600">{formatDate(info.getValue())}</span>,
      }),
      deletionColumnHelper.display({
        id: "actions",
        header: "Actions",
        cell: (info) => (
          <button
            type="button"
            onClick={() => setSelectedDeletionId(info.row.original.id)}
            className="inline-flex rounded-md p-2 text-primary-10 transition-colors hover:bg-neutral-100 cursor-pointer"
            aria-label="View deletion request"
          >
            <Eye className="h-4 w-4" />
          </button>
        ),
  ...
  const deletionsTable = useReactTable({
    data: deletionItems,
    ...
    getPaginationRowModel: getPaginationRowModel(),
    manualPagination: true,
    pageCount: deletionMeta?.totalPages ?? 1,
  });
  ...
  return (
    <section className="flex w-full flex-col gap-5 pt-4 md:pt-0">
      <div>
        <h1 className="font-Raleway text-2xl font-bold text-primary-10">Help & Support</h1>
        ...
        <div className="flex flex-col items-start justify-center overflow-hidden rounded-2xl border border-opacityClr-30 bg-white">
          ...
          <TableHeadAndBody
            ...
            emptyState={{
              image: emptyWallet,
              alt: "No records",
              message: isLoadingDeletions ? "Loading requests..." : "No account deletion requests found.",
            }}
          />
          {deletionItems.length > 0 && (
            <Pagination
              currentPage={deletionMeta?.page ?? deletionsPage}
      ...
      {(selectedContactId || selectedTicketId || selectedDeletionId) && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={closeDetail}>
          <aside className="h-full w-full max-w-xl overflow-y-auto bg-white p-6 shadow-xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                <h2 className="font-Raleway text-xl font-bold text-primary-10">
                  {selectedDeletionId ? "Account deletion request" : selectedTicketId ? "Support ticket" : "Contact request"}
                </h2>
              </div>
            ...
            {selectedDeletionId ? (
              isLoadingDeletionDetail ? (
                <DetailSkeleton />
              ) : deletionDetail ? (
                <div className="flex flex-col gap-5">
                  <div>
                    <h3 className="mt-0 font-Raleway text-lg font-bold text-primary-10">Request from {deletionDetail.email ?? "—"}</h3>
                    <p className="mt-2 text-sm text-opacityClr-80">Requested {formatDate(deletionDetail.createdAt)}</p>
                  </div>
                  <div className="rounded-xl bg-[#F3F4F4] p-4 whitespace-pre-wrap text-sm text-primary-10">
                    {deletionDetail.reason ?? "No reason provided."}
                  </div>
                  <div className="border-t border-opacityClr-20 pt-5">
                    <p className="text-sm">
                      <strong>Status:</strong> {deletionDetail.status ?? "—"}
                    </p>
                  </div>
                </div>
              ) : (
                <p className="text-sm text-opacityClr-80">Unable to load this deletion request.</p>
              )
            ) : (
              (selectedTicketId || selectedContactId) &&
              ((selectedTicketId && isLoadingTicket) || (selectedContactId && isLoadingContact) ? (
                ...
                <div className="flex flex-col gap-5">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-opacityClr-80">{ticket.userEmail ?? "—"}</p>
                  </div>
                    ...
                    {ticket.messages.length ? (
                      <div className="flex flex-col gap-3">
                        {ticket.messages.map((message) => (
                          ...
                          >
                            <p className="mb-1 text-xs font-semibold text-primary-10">
                              {message.authorName ?? (message.author === "support" ? "Support" : "User")}
                            </p>
